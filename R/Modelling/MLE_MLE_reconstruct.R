...
# Define the location and scale formulas including the 'reconstruct' term
location_formula <- if (Pu > 0) {
  as.formula(paste("~ data$reconstruct +", paste(c(paste0("mu", 1:(2*Pu))), collapse = " + ")))
} else {
  ~ reconstruct  # Include 'reconstruct' as the only covariate if Pu = 0
}

scale_formula <- if (Pa > 0) {
  as.formula(paste("~ data$reconstruct +", paste(c(paste0("phi", 1:(2*Pa))), collapse = " + ")))
} else {
  ~ reconstruct  # Include 'reconstruct' as the only covariate if Pa = 0
}
